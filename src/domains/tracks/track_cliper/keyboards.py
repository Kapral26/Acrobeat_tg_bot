...
Модуль `keyboards.py` содержит функции для создания inline-клавиатур,
 связанных с вводом временных периодов для обработки аудиофайлов.
...
async def kb_back_period_input() -> InlineKeyboardMarkup:
    ...
    Используется в контексте обработки аудиообрезки, когда пользователь
     хочет отредактировать ранее введённые временные параметры.
