"""
Модуль `message_cleanup.py` содержит реализацию сервиса для управления
 временными сообщениями, связанными с обработкой аудиообрезки.

Обеспечивает интеграцию с кэширующим репозиторием для хранения
 идентификаторов сообщений, которые требуется удалить после
 завершения операций обработки треков.
"""
