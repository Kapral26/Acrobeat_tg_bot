"""
Модуль `cache_repository.py` содержит реализацию репозитория для
 работы с кэшем временных сообщений, связанных с названиями треков.

Обеспечивает хранение идентификаторов сообщений в Redis
 для последующей очистки после завершения операций.
"""
...
class TrackNameMsgCleanerRepository(BaseMsgCleanerRepository):
    """
    Репозиторий для хранения идентификаторов временных сообщений,
     связанных с вводом названий треков.

    Использует Redis для асинхронного хранения данных.
     Ключи формируются по шаблону `track_name_messages:{user_id}`,
