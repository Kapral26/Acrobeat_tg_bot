"""
Модуль `abstraction.py` определяет абстрактный интерфейс для репозиториев,
 отвечающих за поиск и загрузку музыкальных треков.

Этот интерфейс используется для унификации работы с разными источниками
 (YouTube, Pinkamuz, Telegram и др.), обеспечивая единое API
...
class DownloaderAbstractRepo(ABC):
    """
    Абстрактный класс-интерфейс для всех репозиториев, связанных с загрузкой
     и поиском музыкальных треков.

    Каждый конкретный репозиторий должен реализовать
